...
async function loadDashboard() {
  try {
    ...
    if (todayData.success) {
      ...
      document.getElementById('todayCount').textContent = `${todayData.records.length} records`;
    }
  } catch (err) {
...
function renderTodayList(records) {
  ...
  container.innerHTML = records.slice(0, 15).map((r, i) => `
    <div class="att-item">
...
async function captureAndRecognize() {
  ...
  if (knownFaces.length === 0) {
    showResultPanel('fail', null, 'Koi registered student nahi mila. Pehle students register karein.');
  ...
  if (confidence >= 75) {
    ...
    showResultPanel('fail', null, 'Face recognize nahi ho saka. Dobara try karein.');
  }
}
...
async function markAttendance(studentId, confidence, method) {
  try {
    ...
    if (data.success) {
      ...
      showToast(`✓ ${data.student.name} ki attendance mark ho gayi!`, 'success');
...
function addRecentScan(student, status) {
  recentScans.unshift({ student, status, time: new Date().toLocaleTimeString() });
...
function initRegisterForm() {
  ...
  document.getElementById('captureRegFace').addEventListener('click', () => {
    ...
    faceDescriptorData = Array.from({ length: 128 }, () => Math.random() * 2 - 1);
  ...
  document.getElementById('registerForm').addEventListener('submit', async (e) => {
    ...
    formData.append('studentId', document.getElementById('reg-studentId').value.trim());
    ...
    formData.append('department', document.getElementById('reg-department').value);
    ...
    try {
      ...
      if (data.success) {
        ...
        e.target.reset();
        document.getElementById('faceStatus').innerHTML = '<span class="face-dot red"></span> No face data';
        document.getElementById('facePrev').innerHTML = '<span>◉</span><p>No face captured</p>';
...
function initStudentsPage() {
  ...
  document.getElementById('studentSearch').addEventListener('input', (e) => {
    ...
    searchTimeout = setTimeout(() => loadStudents(e.target.value, document.getElementById('deptFilter').value), 400);
  });
  document.getElementById('deptFilter').addEventListener('change', (e) => {
    loadStudents(document.getElementById('studentSearch').value, e.target.value);
  });
}

async function loadStudents(search = '', department = '') {
  const tbody = document.getElementById('studentsBody');
  tbody.innerHTML = '<tr><td colspan="9" class="loading-cell">Loading students...</td></tr>';
  ...
  try {
    ...
    if (!data.success || !data.students.length) {
      tbody.innerHTML = '<tr><td colspan="9" class="empty-cell">No students found</td></tr>';
      return;
    }
    ...
    showModal('Student Details', `
      <div style="display:grid;gap:12px">
        ...
        ${[
          ...
          ['Class', s.class],
          ['Face Data', s.faceDescriptor?.length ? `✓ Registered (${s.faceDescriptor.length} points)` : '✗ Not registered'],
...
function initRecordsPage() {
  ...
  document.getElementById('filterRecords').addEventListener('click', loadRecords);
...
async function loadRecords() {
  ...
  tbody.innerHTML = '<tr><td colspan="9" class="loading-cell">Loading records...</td></tr>';
  ...
  try {
    ...
    if (!data.success || !data.records.length) {
      tbody.innerHTML = '<tr><td colspan="9" class="empty-cell">No records found</td></tr>';
      return;
    }
...
function exportCSV() {
  const rows = [];
  const headers = ['#', 'Student ID', 'Name', 'Department', 'Date', 'Time', 'Status', 'Method'];
...
async function generateReport() {
  ...
  output.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text3)">Generating report...</div>';
  ...
  try {
    ...
    const pct = records.length > 0 ? Math.round((summary.present / records.length) * 100) : 0;
      ...
      <div style="display:grid;gap:20px">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px">
          ${[
            ...
            ['Attendance %', pct + '%', pct >= 75 ? 'var(--green)' : 'var(--red)']
          ].map(([label, val, color]) => `
        ...
        </div>` : '<div class="empty-state">Is period mein koi record nahi mila</div>'}
      </div>
    `;
  } catch (err) {
